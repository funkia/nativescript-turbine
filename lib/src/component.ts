//import { Monad } from "../../jabz";
import { Now, Behavior, Stream, placeholder } from "@funkia/hareactive";
import { Monad, monad, fgo } from "@funkia/jabz";
 ...
 */
@monad
export abstract class Component<A, P = Node> implements Monad<A> {
  ...
  chain<B>(f: (a: A) => Component<B, P>): Component<B, P> {
    return new ChainComponent(this, f);
  }
  static multi: boolean = false;
  multi: boolean = false;
  abstract run(parent: P): A;
...
class OfComponent<A, P> extends Component<A, P> {
...
class ChainComponent<A, B, P> extends Component<B, P> {
  constructor(
    private component: Component<A, P>,
    private f: (a: A) => Component<B, P>
  ) {
    super();
  }
  run(parent: P): B {
...
class LoopComponent<A, P> extends Component<A, P> {
  constructor(
    private f: (a: A) => Component<A, P>,
...
export function loop<A extends ReactivesObject, P>(
  f:
    | ((a: A) => Component<A, P>)
    | ((a: A) => IterableIterator<Component<any> | A>),
...
export function toComponent<A>(child: Child): Component<any> {
  if (isComponent(child)) {
    ...
    return <Component<A>>sequence(Component, child.map(toComponent)).map((res: any[]) => res.reduce(Object.assign.bind(undefined, {}), {}));
  }
}
