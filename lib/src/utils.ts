...
export function isShowable(s: any): s is Showable {
  return (
    typeof s === "string" || typeof s === "number" || typeof s === "boolean"
  );
}

export type Changeable<A> = A | Behavior<A>;

export function mapChangable<A, B>(
  input: Changeable<A>,
  f: (a: A) => B
): Changeable<B> {
  return isBehavior(input) ? input.map(f) : f(input);
}
